...
import { PageGlow, ArcDecoration } from "@/components/decorative/PageDecorations";
...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload?.length) {
    ...
    return (
      <div className="glass-panel rounded-2xl p-3 shadow-glass text-xs">
        ...
        <Badge variant="secondary" className="mt-1 text-[10px] rounded-full">{clusterMeta[d.cluster as ClusterName]?.label}</Badge>
      </div>
    );
  }
  return null;
...
const MasteryPage = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in relative">
      <PageGlow />
      <ArcDecoration className="-top-10 right-0" />
      <div className="relative z-10 space-y-8">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <Select value={selectedCourse} onValueChange={(v) => { setSelectedCourse(v); setSelectedModule("all"); }}>
              <SelectTrigger className="w-[280px] rounded-full"><SelectValue placeholder="Select course" /></SelectTrigger>
              <SelectContent>
                {courses.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={selectedModule} onValueChange={setSelectedModule}>
              <SelectTrigger className="w-[200px] rounded-full"><SelectValue placeholder="Select module" /></SelectTrigger>
              <SelectContent>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {(Object.entries(clusterMeta) as [ClusterName, typeof clusterMeta[ClusterName]][]).map(([key, info]) => {
            ...
            return (
              <Card key={key} className="relative overflow-hidden hover:shadow-glow transition-all duration-300 group">
                <div className="absolute top-0 left-0 w-full h-1 opacity-60" style={{ background: `linear-gradient(90deg, ${clusterColors[key]}, transparent)` }} />
                <CardContent className="p-5">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="h-3 w-3 rounded-full shadow-sm" style={{ backgroundColor: clusterColors[key] }} />
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={masteryDistribution} layout="vertical">
                  ...
                  <Bar dataKey="mastery" radius={[0, 8, 8, 0]} fill="url(#masteryGrad)" />
                  <defs>
                    <linearGradient id="masteryGrad" x1="0" y1="0" x2="1" y2="0">
                ...
                {course.modules.map(mod => {
                  ...
                  return (
                    <div key={mod.id} className="space-y-2">
                      ...
                      <div className="flex h-3 rounded-full overflow-hidden bg-accent/60">
